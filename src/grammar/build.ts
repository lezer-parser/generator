...
${moduleStyle == "es6" ? `import {s, Parser${sawNoToken ? ", noToken" : ""}} from "../src/parse" // FIXME`
    : `const {s, Parser${sawNoToken ? ", noToken" : ""}} = require("../src/parse")`}
${tokenizerText}
${varText}

s.id = 0
${moduleStyle == "es6" ? `export default` : `module.exports = `} new Parser([\n  ${stateText.join(",\n  ")}\n],\n${JSON.stringify(this.terms.tags)},\n${
JSON.stringify(this.terms.repeatInfo)},\n${JSON.stringify(specialized)},\n${JSON.stringify(specializations)
}${includeNames ? `,\n${JSON.stringify(this.terms.names)}` : ""})`
    // FIXME want prototypes of specializations to be null
  }
