...
class Builder {
  ...
  getParser() {
    ...
    let skipTokenTable = fullSkipAutomata.map(states => states ? states[0].actions.map(a => a.term) : none)
    let fullTable = buildFullAutomaton(this.terms, this.terms.tops, first)
    let {tokenMasks, tokenGroups, tokenPrec} = this.tokens.buildTokenGroups(fullTable, fullSkipAutomata, skipTokenTable)
    let table = finishAutomaton(fullTable) as LRState[]
    ...
    for (let s of table) {
      let skip = noSkip, skipTerms = none as readonly Term[]
      if (s.skip != this.noSkip) {
        let index = this.skipRules.indexOf(s.skip)
        skip = skipData[index]
        skipTerms = skipTokenTable[index]
      }
    ...
    for (let term of this.terms.tops)
      topRules[term.nodeName || "@top"] = [table.find(state => state.topRule == term)!.id, term.id]
  ...
  finishState(state: LRState, tokenizers: any[],
              data: DataBuilder, skipTable: number, skipTerms: readonly Term[], isSkip: boolean,
...
class TokenSet {
  ...
  buildTokenGroups(states: readonly LRState[], skipStates: readonly (LRState[] | null)[], skipTokens: readonly (readonly Term[])[]) {
    ...
    let usedPrec: Term[] = []
    let conflicts = tokens.findConflicts(checkTogether(states, this.b, skipTokens)).filter(({a, b}) => {
    ...
    let checkState = (state: LRState) => {
      ...
      let skip = state.skip == this.b.noSkip ? none : skipTokens[this.b.skipRules.indexOf(state.skip)]
...
function checkTogether(states: readonly LRState[], b: Builder, skipTokens: readonly (readonly Term[])[]) {
  ...
  function hasTerm(state: LRState, term: Term) {
    return state.actions.some(a => a.term == term) ||
      state.skip != b.noSkip && skipTokens[b.skipRules.indexOf(state.skip)].includes(term)
  }
