...
class Builder {
  ...
  normalizeRepeat(expr: RepeatExpression) {
    ...
    let inner = this.newName(name, true, {repeated: true})
    inner.repeated = true
...
${encodeArray((end as LezerTokenGroup).data)}, ${placeholder}]`
  ...
  let nodeInfo: any[] = [], builtinProps: string[] = []
  for (let type of parser.group.types) {
    nodeInfo.push(JSON.stringify(type.name))
    let propData = (type as any).props.propData
    for (let i = 0; i < propData.length; i += 2) {
      let source = propData[i], value = propData[i + 1], propID
      if (source.from == null) {
        if (!builtinProps.includes(source.name)) builtinProps.push(source.name)
        propID = source.name // FIXME name clashes?
      } else {
  ...
  let builtinImport = !builtinProps.length ? "" :
    `const {${builtinProps.join(", ")}} = ${importName("NodeProp", "lezer", "NodeProp")}\n`
