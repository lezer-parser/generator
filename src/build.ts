...
import {Parser, TagMap, ParseState, TokenGroup as LezerTokenGroup, ExternalTokenizer,
        REDUCE_DEPTH_SHIFT, REDUCE_FLAG, REDUCE_TERM_MASK, REDUCE_REPEAT_FLAG, ACCEPTING, SPECIALIZE, EXTEND,
        TERM_ERR, STAY_FLAG} from "lezer"
  ...
  getParser() {
    ...
    let skipStartStates = fullSkipAutomata.map((states, i) => {
      ...
      for (let state of states) {
        if (state.id != 0) table.push(state)
    ...
    let skipData = skipStartStates.map((state, i) => {
      let actions: number[] = []
      if (state) for (let action of state.actions as Shift[]) {
        // FIXME more well-defined check for end-of-skip state
        if (action.target.set.some(pos => pos.rule.name == this.skipRules[i]) && action.target.defaultReduce)
